...
1. **Distinguir** `categoría de forecast`, `precisión histórica`, `supuesto de la proyección` y `descuento mental` por sus observables y no por su definición memorizada.
2. **Explicar** por qué esas distinciones cambian una decisión concreta dentro de **Dirección comercial: CMO, VP Sales y CRO**.
3. **Aplicar** la secuencia **clasificar el pipeline en categorías con criterios → declarar el método y sus supuestos → presentar la precisión histórica junto a la proyección → identificar los negocios que concentran el riesgo → actualizar con frecuencia definida y registrar los cambios** conservando supuestos, alternativas descartadas y trazabilidad.
4. **Operacionalizar** **precisión histórica del forecast**, **concentración del riesgo** y **variación entre actualizaciones** indicando numerador, denominador, ventana, fuente y uso permitido.
...
**categoría de forecast** se entiende aquí como **clasificación entre comprometido, probable y mejor caso**. Es la pieza desde la que se inicia el análisis de forecast ejecutivo: antes de «clasificar el pipeline en categorías con criterios», hay que poder señalar qué cambia en la operación si el concepto está presente y qué debería observarse si no lo está.

La lectura rectora de este bloque es Andrew S. Grove — *High Output Management* (1983). **Lente que aporta:** output gerencial, indicadores adelantados y reuniones como herramienta de producción. Úsala sin convertirla en dogma: escribe una proposición de la obra que apoye tu diagnóstico, una condición del caso que la limite y una consecuencia práctica. La evidencia mínima es **precisión histórica del forecast**; regístrala con periodo, unidad, población y línea base.
...
**Definición operacional:** registro de la desviación entre proyección y resultado en periodos anteriores. Su valor está en distinguirlo de **categoría de forecast**. En una decisión real, clasificar mal una situación cambia la intervención: se asigna presupuesto donde faltaba diagnóstico, se mide un resultado cuando había que observar un proceso, o se trata una restricción como si fuera una preferencia.

Contrasta el problema con Mark Roberge — *The Sales Acceleration Formula* (2015) —**lente:** contratación, formación, gestión y demanda comercial gobernadas por datos—. Formula dos mini-casos: uno que satisface la definición de **precisión histórica** y otro que sólo se le parece en la superficie. Después pregunta qué señal los distingue; **concentración del riesgo** es candidata, pero debe combinarse con evidencia cualitativa cuando el fenómeno no es directamente medible.
...
**Definición:** ajuste informal que cada ejecutivo aplica a un forecast poco confiable. Este concepto obliga a abandonar la idea de que forecast ejecutivo tiene una solución gratuita. Toda intervención consume caja, tiempo, atención del equipo, capacidad de la operación, reputación o tolerancia al riesgo. Por eso, antes de «identificar los negocios que concentran el riesgo», se comparan al menos dos alternativas plausibles y se explicita qué se sacrifica en cada una.

Robert S. Kaplan y David P. Norton — *The Balanced Scorecard* (1996) —**lente:** traducción de la estrategia en indicadores causalmente conectados— sirve para construir una matriz `beneficio esperado / costo / reversibilidad / stakeholder afectado / señal temprana`. La evidencia **variación entre actualizaciones** ayuda a detectar si el trade-off está ocurriendo como se esperaba, pero no elimina la obligación de observar efectos laterales fuera del indicador principal.
...
Robert S. Kaplan y David P. Norton — *The Balanced Scorecard* (1996) sirve para contrastar la recomendación final desde otro lente: traducción de la estrategia en indicadores causalmente conectados. La frontera de esta clase es explícita: Un forecast conservador sistemático también es un problema: induce decisiones de inversión más tímidas que las que el negocio permite. Conviértela en una regla operativa con el formato `si ocurre X → no aplicar automáticamente → consultar, escalar o revalidar`.
...
2. Ejecuta la secuencia **clasificar el pipeline en categorías con criterios → declarar el método y sus supuestos → presentar la precisión histórica junto a la proyección → identificar los negocios que concentran el riesgo → actualizar con frecuencia definida y registrar los cambios** y adjunta evidencia en cada transición.
3. Construye la ficha de medición de **precisión histórica del forecast**; si el dato no existe, diseña cómo obtenerlo y cuánto costaría.
...
1. Explica la diferencia entre **categoría de forecast** y **precisión histórica** con un ejemplo donde elegir mal cambie la decisión.
2. ¿Qué observarías para validar **supuesto de la proyección** y qué observación te obligaría a rechazar tu interpretación?
3. Aplica «clasificar el pipeline en categorías con criterios» al caso de la clase. ¿Qué dato sigue faltando?
4. ¿Por qué **precisión histórica del forecast** no basta por sí sola para atribuir causalidad?
5. Compara dos fuentes de la lectura comparada: ¿dónde llevarían a recomendaciones distintas?
6. ¿Qué decisión equivocada se produciría si se ignora este límite: «Un forecast conservador sistemático también es un problema: induce decisiones de inversión más tímidas que las que el negocio permite»?
...
- `ficha-metricas.md` — definición operacional de **precisión histórica del forecast**, **concentración del riesgo** y **variación entre actualizaciones** con fuente, ventana y lectura prohibida.
...
← [Clase 07 · OKR y KPI](class-07-okr-y-kpi.md) · [Índice de la parte](README.md) · [Clase 09 · Reuniones operativas](class-09-reuniones-operativas.md) →
