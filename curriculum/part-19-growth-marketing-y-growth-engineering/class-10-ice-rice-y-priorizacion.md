...
**impacto estimado** se entiende aquí como **efecto esperado sobre la métrica objetivo si la hipótesis es correcta**. Es la pieza desde la que se inicia el análisis de ICE, RICE y priorización: antes de «definir la escala de cada componente con criterios», hay que poder señalar qué cambia en la operación si el concepto está presente y qué debería observarse si no lo está.

La lectura rectora de este bloque es Sean Ellis y Morgan Brown — *Hacking Growth* (2017). **Lente que aporta:** equipo multifuncional, ciclo de experimentación y aha moment. Úsala sin convertirla en dogma: escribe una proposición de la obra que apoye tu diagnóstico, una condición del caso que la limite y una consecuencia práctica. La evidencia mínima es **calibración de estimaciones**; regístrala con periodo, unidad, población y línea base.
...
**Definición operacional:** grado de evidencia que respalda la expectativa de impacto. Su valor está en distinguirlo de **impacto estimado**. En una decisión real, clasificar mal una situación cambia la intervención: se asigna presupuesto donde faltaba diagnóstico, se mide un resultado cuando había que observar un proceso, o se trata una restricción como si fuera una preferencia.

Contrasta el problema con Douglas W. Hubbard — *How to Measure Anything* (2014, 3.ª ed.) —**lente:** medir lo que parece inmedible: valor de la información y reducción de incertidumbre—. Formula dos mini-casos: uno que satisface la definición de **confianza** y otro que sólo se le parece en la superficie. Después pregunta qué señal los distingue; **dispersión entre evaluadores** es candidata, pero debe combinarse con evidencia cualitativa cuando el fenómeno no es directamente medible.
...
**esfuerzo** significa **recursos necesarios para ejecutar, expresados en una unidad comparable**. El problema ya no es definirlo sino medirlo: qué contar, en qué ventana, con qué denominador, contra qué línea base y con qué segmentación. Una métrica útil conserva contexto suficiente para no confundir una mejora local con una mejora del sistema.
...
**Definición:** apariencia de rigor que produce un puntaje basado en estimaciones subjetivas. Este concepto obliga a abandonar la idea de que ICE, RICE y priorización tiene una solución gratuita. Toda intervención consume caja, tiempo, atención del equipo, capacidad de la operación, reputación o tolerancia al riesgo. Por eso, antes de «ejecutar en orden y registrar el resultado», se comparan al menos dos alternativas plausibles y se explicita qué se sacrifica en cada una.

Foster Provost y Tom Fawcett — *Data Science for Business* (2013) —**lente:** pensamiento analítico: formulación del problema, evaluación y valor esperado— sirve para construir una matriz `beneficio esperado / costo / reversibilidad / stakeholder afectado / señal temprana`. La evidencia **orden de ejecución respetado** ayuda a detectar si el trade-off está ocurriendo como se esperaba, pero no elimina la obligación de observar efectos laterales fuera del indicador principal.
...
Foster Provost y Tom Fawcett — *Data Science for Business* (2013) sirve para contrastar la recomendación final desde otro lente: pensamiento analítico: formulación del problema, evaluación y valor esperado. La frontera de esta clase es explícita: Ningún marco reemplaza el juicio estratégico. Iniciativas de alto valor y alto esfuerzo pueden quedar postergadas indefinidamente por un puntaje. Conviértela en una regla operativa con el formato `si ocurre X → no aplicar automáticamente → consultar, escalar o revalidar`.
...
2. Ejecuta la secuencia **definir la escala de cada componente con criterios → puntuar con participación de más de una persona → revisar los casos donde el puntaje contradice la intuición → ejecutar en orden y registrar el resultado → calibrar las estimaciones con los resultados observados** y adjunta evidencia en cada transición.
3. Construye la ficha de medición de **calibración de estimaciones**; si el dato no existe, diseña cómo obtenerlo y cuánto costaría.
