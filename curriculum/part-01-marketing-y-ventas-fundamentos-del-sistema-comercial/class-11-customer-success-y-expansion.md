...
1. **Distinguir** `resultado deseado del cliente`, `time-to-value`, `salud de cuenta` y `expansión legítima` por sus observables y no por su definición memorizada.
...
3. **Aplicar** la secuencia **definir el resultado deseado con el cliente antes de implementar → instrumentar la señal que evidencia ese resultado → medir el tiempo hasta el primer valor → revisar salud de cuenta con criterios y no con impresiones → proponer expansión sólo cuando el resultado inicial está acreditado** conservando supuestos, alternativas descartadas y trazabilidad.
4. **Operacionalizar** **time-to-value mediano**, **ingreso neto retenido** y **cobertura de resultado documentado** indicando numerador, denominador, ventana, fuente y uso permitido.
...
**resultado deseado del cliente** se entiende aquí como **cambio concreto que el cliente esperaba lograr, expresado en su métrica y no en la del proveedor**. Es la pieza desde la que se inicia el análisis de customer Success y expansión: antes de «definir el resultado deseado con el cliente antes de implementar», hay que poder señalar qué cambia en la operación si el concepto está presente y qué debería observarse si no lo está.

La lectura rectora de este bloque es Nick Mehta, Dan Steinman y Lincoln Murphy — *Customer Success* (2016). **Lente que aporta:** disciplina operativa de éxito de cliente: salud, renovación y expansión. Úsala sin convertirla en dogma: escribe una proposición de la obra que apoye tu diagnóstico, una condición del caso que la limite y una consecuencia práctica. La evidencia mínima es **time-to-value mediano**; regístrala con periodo, unidad, población y línea base.
...
**Definición operacional:** tiempo entre la firma y la primera evidencia verificable de ese resultado. Su valor está en distinguirlo de **resultado deseado del cliente**. En una decisión real, clasificar mal una situación cambia la intervención: se asigna presupuesto donde faltaba diagnóstico, se mide un resultado cuando había que observar un proceso, o se trata una restricción como si fuera una preferencia.

Contrasta el problema con Samuel Hulick — *The Elements of User Onboarding* (2014) —**lente:** diseño del primer valor percibido y reducción del time-to-value—. Formula dos mini-casos: uno que satisface la definición de **time-to-value** y otro que sólo se le parece en la superficie. Después pregunta qué señal los distingue; **ingreso neto retenido** es candidata, pero debe combinarse con evidencia cualitativa cuando el fenómeno no es directamente medible.
...
**salud de cuenta** significa **estimación del riesgo de baja construida con uso, resultado, relación y señales comerciales**. El problema ya no es definirlo sino medirlo: qué contar, en qué ventana, con qué denominador, contra qué línea base y con qué segmentación. Una métrica útil conserva contexto suficiente para no confundir una mejora local con una mejora del sistema.
...
**Definición:** aumento de ingreso en una cuenta que ya obtuvo el resultado inicial comprometido. Este concepto obliga a abandonar la idea de que customer Success y expansión tiene una solución gratuita. Toda intervención consume caja, tiempo, atención del equipo, capacidad de la operación, reputación o tolerancia al riesgo. Por eso, antes de «revisar salud de cuenta con criterios y no con impresiones», se comparan al menos dos alternativas plausibles y se explicita qué se sacrifica en cada una.

Matthew Dixon, Nick Toman y Rick DeLisi — *The Effortless Experience* (2013) —**lente:** reducción del esfuerzo del cliente como motor de lealtad frente al deleite— sirve para construir una matriz `beneficio esperado / costo / reversibilidad / stakeholder afectado / señal temprana`. La evidencia **cobertura de resultado documentado** ayuda a detectar si el trade-off está ocurriendo como se esperaba, pero no elimina la obligación de observar efectos laterales fuera del indicador principal.
...
2. Ejecuta la secuencia **definir el resultado deseado con el cliente antes de implementar → instrumentar la señal que evidencia ese resultado → medir el tiempo hasta el primer valor → revisar salud de cuenta con criterios y no con impresiones → proponer expansión sólo cuando el resultado inicial está acreditado** y adjunta evidencia en cada transición.
3. Construye la ficha de medición de **time-to-value mediano**; si el dato no existe, diseña cómo obtenerlo y cuánto costaría.
...
1. Explica la diferencia entre **resultado deseado del cliente** y **time-to-value** con un ejemplo donde elegir mal cambie la decisión.
2. ¿Qué observarías para validar **salud de cuenta** y qué observación te obligaría a rechazar tu interpretación?
3. Aplica «definir el resultado deseado con el cliente antes de implementar» al caso de la clase. ¿Qué dato sigue faltando?
4. ¿Por qué **time-to-value mediano** no basta por sí sola para atribuir causalidad?
5. Compara dos fuentes de la lectura comparada: ¿dónde llevarían a recomendaciones distintas?
6. ¿Qué decisión equivocada se produciría si se ignora este límite: «Customer Success no puede compensar un producto que no resuelve el problema ni una venta que prometió lo que no existe. Cuando el churn se concentra en un segmento específico, el diagnóstico probablemente esté en oferta o en calificación»?
...
- `ficha-metricas.md` — definición operacional de **time-to-value mediano**, **ingreso neto retenido** y **cobertura de resultado documentado** con fuente, ventana y lectura prohibida.
...
← [Clase 10 · Ventas transaccionales y consultivas](class-10-ventas-transaccionales-y-consultivas.md) · [Índice de la parte](README.md) · [Clase 12 · Revenue Operations como integración](class-12-revenue-operations-como-integracion.md) →
