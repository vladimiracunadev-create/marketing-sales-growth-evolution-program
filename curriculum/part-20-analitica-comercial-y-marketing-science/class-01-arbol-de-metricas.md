...
**descomposición multiplicativa** se entiende aquí como **expresión del resultado como producto de factores medibles**. Es la pieza desde la que se inicia el análisis de árbol de métricas: antes de «definir el resultado en la cima del árbol», hay que poder señalar qué cambia en la operación si el concepto está presente y qué debería observarse si no lo está.

La lectura rectora de este bloque es Robert S. Kaplan y David P. Norton — *The Balanced Scorecard* (1996). **Lente que aporta:** traducción de la estrategia en indicadores causalmente conectados. Úsala sin convertirla en dogma: escribe una proposición de la obra que apoye tu diagnóstico, una condición del caso que la limite y una consecuencia práctica. La evidencia mínima es **cobertura del árbol**; regístrala con periodo, unidad, población y línea base.
...
**Definición operacional:** factor del árbol que alguien puede modificar con una decisión. Su valor está en distinguirlo de **descomposición multiplicativa**. En una decisión real, clasificar mal una situación cambia la intervención: se asigna presupuesto donde faltaba diagnóstico, se mide un resultado cuando había que observar un proceso, o se trata una restricción como si fuera una preferencia.

Contrasta el problema con Alistair Croll y Benjamin Yoskovitz — *Lean Analytics* (2013) —**lente:** una métrica que importa por etapa y por modelo de negocio—. Formula dos mini-casos: uno que satisface la definición de **variable accionable** y otro que sólo se le parece en la superficie. Después pregunta qué señal los distingue; **consistencia aritmética** es candidata, pero debe combinarse con evidencia cualitativa cuando el fenómeno no es directamente medible.
...
**nivel de agregación** significa **grado de detalle en que se descompone cada rama**. El problema ya no es definirlo sino medirlo: qué contar, en qué ventana, con qué denominador, contra qué línea base y con qué segmentación. Una métrica útil conserva contexto suficiente para no confundir una mejora local con una mejora del sistema.
...
**Definición:** relación explícita entre un factor y el resultado que afecta. Este concepto obliga a abandonar la idea de que árbol de métricas tiene una solución gratuita. Toda intervención consume caja, tiempo, atención del equipo, capacidad de la operación, reputación o tolerancia al riesgo. Por eso, antes de «asignar responsable a cada rama», se comparan al menos dos alternativas plausibles y se explicita qué se sacrifica en cada una.

Avinash Kaushik — *Web Analytics 2.0* (2009) —**lente:** medición orientada a decisión, segmentación y crítica del dato de vanidad— sirve para construir una matriz `beneficio esperado / costo / reversibilidad / stakeholder afectado / señal temprana`. La evidencia **ramas con responsable** ayuda a detectar si el trade-off está ocurriendo como se esperaba, pero no elimina la obligación de observar efectos laterales fuera del indicador principal.
...
Avinash Kaushik — *Web Analytics 2.0* (2009) sirve para contrastar la recomendación final desde otro lente: medición orientada a decisión, segmentación y crítica del dato de vanidad. La frontera de esta clase es explícita: No todos los factores son multiplicativos ni independientes. Un árbol demasiado simple puede esconder interacciones importantes entre variables. Conviértela en una regla operativa con el formato `si ocurre X → no aplicar automáticamente → consultar, escalar o revalidar`.
...
2. Ejecuta la secuencia **definir el resultado en la cima del árbol → descomponer en factores multiplicativos verificables → continuar hasta llegar a variables accionables → asignar responsable a cada rama → verificar la aritmética con datos reales** y adjunta evidencia en cada transición.
3. Construye la ficha de medición de **cobertura del árbol**; si el dato no existe, diseña cómo obtenerlo y cuánto costaría.
