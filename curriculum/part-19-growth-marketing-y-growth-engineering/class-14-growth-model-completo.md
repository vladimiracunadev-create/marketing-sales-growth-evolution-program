...
sources: ["ellis-brown", "croll-yoskovitz", "provost", "kohavi"]
updated: 2026-08-19
---
...
**modelo de crecimiento** se entiende aquí como **representación cuantitativa de las palancas y su efecto en el resultado**. Es la pieza desde la que se inicia el análisis de growth model completo: antes de «construir el modelo con las palancas y sus relaciones», hay que poder señalar qué cambia en la operación si el concepto está presente y qué debería observarse si no lo está.

La lectura rectora de este bloque es Sean Ellis y Morgan Brown — *Hacking Growth* (2017). **Lente que aporta:** equipo multifuncional, ciclo de experimentación y aha moment. Úsala sin convertirla en dogma: escribe una proposición de la obra que apoye tu diagnóstico, una condición del caso que la limite y una consecuencia práctica. La evidencia mínima es **precisión del modelo**; regístrala con periodo, unidad, población y línea base.
...
**Definición operacional:** variable que el equipo puede modificar y que afecta la métrica estrella. Su valor está en distinguirlo de **modelo de crecimiento**. En una decisión real, clasificar mal una situación cambia la intervención: se asigna presupuesto donde faltaba diagnóstico, se mide un resultado cuando había que observar un proceso, o se trata una restricción como si fuera una preferencia.

Contrasta el problema con Alistair Croll y Benjamin Yoskovitz — *Lean Analytics* (2013) —**lente:** una métrica que importa por etapa y por modelo de negocio—. Formula dos mini-casos: uno que satisface la definición de **palanca** y otro que sólo se le parece en la superficie. Después pregunta qué señal los distingue; **palancas con efecto verificado** es candidata, pero debe combinarse con evidencia cualitativa cuando el fenómeno no es directamente medible.
...
**sensibilidad del modelo** significa **efecto en el resultado de variar cada palanca**. El problema ya no es definirlo sino medirlo: qué contar, en qué ventana, con qué denominador, contra qué línea base y con qué segmentación. Una métrica útil conserva contexto suficiente para no confundir una mejora local con una mejora del sistema.
...
**Definición:** documentación acumulada de experimentos y sus conclusiones. Este concepto obliga a abandonar la idea de que growth model completo tiene una solución gratuita. Toda intervención consume caja, tiempo, atención del equipo, capacidad de la operación, reputación o tolerancia al riesgo. Por eso, antes de «priorizar el backlog con base en el modelo», se comparan al menos dos alternativas plausibles y se explicita qué se sacrifica en cada una.

Ron Kohavi, Diane Tang y Ya Xu — *Trustworthy Online Controlled Experiments* (2020) —**lente:** diseño estadístico de experimentos, métricas guardrail y trampas de interpretación— sirve para construir una matriz `beneficio esperado / costo / reversibilidad / stakeholder afectado / señal temprana`. La evidencia **aprendizajes acumulados** ayuda a detectar si el trade-off está ocurriendo como se esperaba, pero no elimina la obligación de observar efectos laterales fuera del indicador principal.
...
Ron Kohavi, Diane Tang y Ya Xu — *Trustworthy Online Controlled Experiments* (2020) sirve para contrastar la recomendación final desde otro lente: diseño estadístico de experimentos, métricas guardrail y trampas de interpretación. La frontera de esta clase es explícita: Un modelo con supuestos no verificados produce proyecciones falsas. Su valor depende de cuántas relaciones fueron medidas y no supuestas. Conviértela en una regla operativa con el formato `si ocurre X → no aplicar automáticamente → consultar, escalar o revalidar`.
...
2. Ejecuta la secuencia **construir el modelo con las palancas y sus relaciones → calibrar con datos históricos → ejecutar el análisis de sensibilidad → priorizar el backlog con base en el modelo → actualizar el modelo con cada aprendizaje** y adjunta evidencia en cada transición.
3. Construye la ficha de medición de **precisión del modelo**; si el dato no existe, diseña cómo obtenerlo y cuánto costaría.
...
1. Explica la diferencia entre **modelo de crecimiento** y **palanca** con un ejemplo donde elegir mal cambie la decisión.
2. ¿Qué observarías para validar **sensibilidad del modelo** y qué observación te obligaría a rechazar tu interpretación?
3. Aplica «construir el modelo con las palancas y sus relaciones» al caso de la clase. ¿Qué dato sigue faltando?
4. ¿Por qué **precisión del modelo** no basta por sí sola para atribuir causalidad?
5. Compara dos fuentes de la lectura comparada: ¿dónde llevarían a recomendaciones distintas?
6. ¿Qué decisión equivocada se produciría si se ignora este límite: «Un modelo con supuestos no verificados produce proyecciones falsas. Su valor depende de cuántas relaciones fueron medidas y no supuestas»?
...
← [Clase 13 · Product-led growth](class-13-product-led-growth.md) · [Índice de la parte](README.md)
