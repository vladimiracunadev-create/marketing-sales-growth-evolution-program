...
**campo crítico** se entiende aquí como **dato sin el cual no puede calcularse forecast ni conversión**. Es la pieza desde la que se inicia el análisis de disciplina de CRM: antes de «definir el conjunto mínimo de campos críticos», hay que poder señalar qué cambia en la operación si el concepto está presente y qué debería observarse si no lo está.

La lectura rectora de este bloque es Mark Roberge — *The Sales Acceleration Formula* (2015). **Lente que aporta:** contratación, formación, gestión y demanda comercial gobernadas por datos. Úsala sin convertirla en dogma: escribe una proposición de la obra que apoye tu diagnóstico, una condición del caso que la limite y una consecuencia práctica. La evidencia mínima es **completitud de campos críticos**; regístrala con periodo, unidad, población y línea base.
...
**Definición operacional:** conjunto de reglas que mantiene el registro completo, actualizado y sin duplicados. Su valor está en distinguirlo de **campo crítico**. En una decisión real, clasificar mal una situación cambia la intervención: se asigna presupuesto donde faltaba diagnóstico, se mide un resultado cuando había que observar un proceso, o se trata una restricción como si fuera una preferencia.

Contrasta el problema con Stephen G. Diorio y Chris K. Hummel — *Revenue Operations* (2022) —**lente:** integración de datos, procesos y equipos que producen ingreso como un solo sistema—. Formula dos mini-casos: uno que satisface la definición de **higiene de datos** y otro que sólo se le parece en la superficie. Después pregunta qué señal los distingue; **oportunidad del registro** es candidata, pero debe combinarse con evidencia cualitativa cuando el fenómeno no es directamente medible.
...
**valor devuelto al usuario** significa **beneficio concreto que el vendedor obtiene del sistema por registrar bien**. El problema ya no es definirlo sino medirlo: qué contar, en qué ventana, con qué denominador, contra qué línea base y con qué segmentación. Una métrica útil conserva contexto suficiente para no confundir una mejora local con una mejora del sistema.
...
**Definición:** actualización realizada dentro del plazo definido tras cada interacción. Este concepto obliga a abandonar la idea de que disciplina de CRM tiene una solución gratuita. Toda intervención consume caja, tiempo, atención del equipo, capacidad de la operación, reputación o tolerancia al riesgo. Por eso, antes de «medir completitud y oportunidad del registro», se comparan al menos dos alternativas plausibles y se explicita qué se sacrifica en cada una.

Foster Provost y Tom Fawcett — *Data Science for Business* (2013) —**lente:** pensamiento analítico: formulación del problema, evaluación y valor esperado— sirve para construir una matriz `beneficio esperado / costo / reversibilidad / stakeholder afectado / señal temprana`. La evidencia **duplicados detectados** ayuda a detectar si el trade-off está ocurriendo como se esperaba, pero no elimina la obligación de observar efectos laterales fuera del indicador principal.
...
Foster Provost y Tom Fawcett — *Data Science for Business* (2013) sirve para contrastar la recomendación final desde otro lente: pensamiento analítico: formulación del problema, evaluación y valor esperado. La frontera de esta clase es explícita: Más campos no es más control: cada campo obligatorio adicional reduce la calidad del conjunto. La disciplina se sostiene con pocos campos que realmente se usan. Conviértela en una regla operativa con el formato `si ocurre X → no aplicar automáticamente → consultar, escalar o revalidar`.
...
2. Ejecuta la secuencia **definir el conjunto mínimo de campos críticos → eliminar los campos que nadie usa → devolver valor al vendedor con vistas y alertas útiles → medir completitud y oportunidad del registro → corregir el proceso antes de sancionar a la persona** y adjunta evidencia en cada transición.
3. Construye la ficha de medición de **completitud de campos críticos**; si el dato no existe, diseña cómo obtenerlo y cuánto costaría.
