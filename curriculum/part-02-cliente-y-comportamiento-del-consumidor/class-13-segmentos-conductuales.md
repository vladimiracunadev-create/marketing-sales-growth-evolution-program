...
Vienes de la clase 02.12, *Contexto cultural y social*: ten a mano su entregable, porque esta sesión lo retoma y lo lleva más lejos.

Trabajarás sobre el caso de la clase. Si prefieres usar datos de tu organización, lo mínimo que necesitas es una serie histórica de tamaño y estabilidad del segmento con la que calcular una línea base: sin ella podrás discutir el concepto, pero no comprobar si tu decisión mejora algo. Ten también dónde escribir —planilla o cuaderno— y, de la lectura comparada, al menos el índice y los capítulos que se indican al pie.
...
**Segmento conductual** se entiende aquí como **grupo definido por acciones registradas y no por características declaradas**.

Un segmento conductual se define por lo que las personas hicieron, no por lo que son. Esa diferencia importa porque la conducta pasada es el mejor predictor disponible de la conducta futura, mientras que los atributos declarados predicen poco. Un grupo definido como «compró tres veces en los últimos seis meses» permite decidir una acción concreta; uno definido como «profesionales jóvenes urbanos» casi nunca la permite.

**De dónde viene esta afirmación.** Peter Fader — *Customer Centricity* (2020, 2.ª ed.) aporta la idea que sostiene este bloque: recencia, frecuencia y valor como base de segmentación conductual. Búscala en los capítulos sobre segmentación por comportamiento. Aplicada a esta clase, esa idea predice algo verificable: si es correcta, «tamaño y estabilidad del segmento» debería moverse cuando cambie **segmento conductual**, y no debería moverse cuando cambie el resto. Ese es el contraste que tienes que montar antes de recomendar nada.
...
**Definición operacional:** tres dimensiones básicas de comportamiento de compra que ordenan la base de clientes. Su valor está en distinguirlo de **segmento conductual**.

Recencia, frecuencia y valor funcionan porque capturan tres dimensiones que no se sustituyen entre sí. Un cliente que gastó mucho hace dos años no es equivalente a uno que gasta poco todos los meses, y tratarlos igual —como hace cualquier segmentación por gasto acumulado— produce campañas que molestan a unos y desaprovechan a otros. La combinación de las tres variables es lo que da la señal; ninguna aislada la da.

**Contraste bibliográfico.** Alistair Croll y Benjamin Yoskovitz — *Lean Analytics* (2013) aporta aquí una distinción concreta: el análisis de cohortes como corrección al promedio que esconde la mezcla (el capítulo sobre cohortes y segmentación). Formula dos mini-casos: uno que satisface la definición de **recencia, frecuencia y valor** y otro que sólo se le parece en la superficie; después decide cuál de los dos describiría esa obra con su propio vocabulario. Si la obra no permite separarlos, la distinción es tuya y tienes que sostenerla con evidencia del caso, no con la cita.
...
**Cohorte** significa **conjunto de clientes que comparten el mismo periodo de inicio y que se sigue en el tiempo**.

Un segmento es accionable sólo si se cumplen tres condiciones verificables: se puede identificar a sus miembros en el sistema, se puede alcanzar por algún canal disponible y existe una acción distinta que ejecutar con ellos. Documentar las tres antes de crear el segmento evita el resultado más común de un ejercicio de segmentación: una presentación con seis grupos elegantes que nadie puede operar.
...
**Definición:** posibilidad real de alcanzar y tratar de forma distinta a ese grupo.

Más segmentos permiten mayor pertinencia y consumen capacidad de ejecución que casi nunca se estima. Cada segmento adicional exige contenido propio, reglas propias y medición propia. Una regla práctica útil es no crear más segmentos de los que el equipo puede atender con calidad en un trimestre, y ordenar el resto en una lista de espera explícita.

**Lo que aporta la fuente.** Kevin Hillstrom — *Hillstrom's Multichannel Forensics* (2007) aporta el criterio para pesar el intercambio: los patrones de recompra que distinguen un negocio de compra única de uno recurrente (los capítulos sobre ciclos de compra). Úsalo para construir una matriz `beneficio esperado / costo / reversibilidad / afectado / señal temprana`. La evidencia **valor promedio por segmento** ayuda a detectar si el intercambio está ocurriendo como se esperaba, pero no elimina la obligación de observar efectos laterales fuera del indicador principal.
...
La segmentación conductual describe patrones agregados y su validez decae con el tiempo. Un segmento construido antes de un cambio de precio o de una campaña masiva puede haber dejado de existir sin que nadie lo note, porque el sistema sigue calculándolo. La regla operativa es fijar una frecuencia de recálculo y una condición de revisión extraordinaria ante cambios relevantes.

**Frontera declarada.** Los segmentos conductuales cambian con el producto: cada cambio relevante de funcionalidad puede invalidar la segmentación y obliga a recalcularla. Conviértela en una regla operativa con el formato `si ocurre X → no aplicar automáticamente → consultar, escalar o revalidar`.
...
|---|---|---|---|
| Peter Fader — *Customer Centricity* (2020, 2.ª ed.) | Recencia, frecuencia y valor como base de segmentación conductual | Los capítulos sobre segmentación por comportamiento | ¿Qué debería observarse en **segmento conductual** si aquí opera «recencia, frecuencia y valor como base de segmentación conductual»? ¿Y qué observación lo desmentiría en este caso? |
| Alistair Croll y Benjamin Yoskovitz — *Lean Analytics* (2013) | El análisis de cohortes como corrección al promedio que esconde la mezcla | El capítulo sobre cohortes y segmentación | ¿Qué debería observarse en **recencia, frecuencia y valor** si aquí opera «el análisis de cohortes como corrección al promedio que esconde la mezcla»? ¿Y qué observación lo desmentiría en este caso? |
| Avinash Kaushik — *Web Analytics 2.0* (2009) | La segmentación como condición para que un promedio signifique algo | El capítulo sobre segmentación de datos | ¿Qué debería observarse en **cohorte** si aquí opera «la segmentación como condición para que un promedio signifique algo»? ¿Y qué observación lo desmentiría en este caso? |
| Kevin Hillstrom — *Hillstrom's Multichannel Forensics* (2007) | Los patrones de recompra que distinguen un negocio de compra única de uno recurrente | Los capítulos sobre ciclos de compra | ¿Qué debería observarse en **accionabilidad del segmento** si aquí opera «los patrones de recompra que distinguen un negocio de compra única de uno recurrente»? ¿Y qué observación lo desmentiría en este caso? |
...
| 2 | **Ejecutar el método** | Recorre la secuencia **definir las acciones que se registrarán como señal → construir los segmentos con datos existentes → verificar que cada segmento sea alcanzable y suficientemente grande → diseñar un tratamiento distinto por segmento → medir el efecto diferencial y descartar segmentos sin respuesta** y adjunta la evidencia usada en cada transición. | La tabla del paso 1 | Cada paso deja un artefacto revisable y una alternativa descartada con su razón. |
| 3 | **Operacionalizar la señal** | Construye la ficha de medición de **tamaño y estabilidad del segmento**; si el dato no existe, diseña cómo obtenerlo y estima cuánto costaría. | Fuentes de datos reales o el diseño de captura | Dos personas del equipo calculan el mismo número con la ficha y llegan al mismo resultado. |
...
| 2 | Dos observaciones concretas: una que confirmaría **cohorte** y otra que te obligaría a abandonarlo. Una respuesta sin condición de refutación no es suficiente. |
| 3 | El dato faltante debe ser nombrable y obtenible: qué se mide, quién lo tiene y en cuánto tiempo. «Faltan datos» no cuenta como respuesta. |
| 4 | Debes distinguir asociación de causa y proponer al menos una explicación alternativa del mismo movimiento de **tamaño y estabilidad del segmento**. |
| 5 | Identifica la condición del caso que decide entre ambas obras. Basta con que sea una: la respuesta correcta no es «depende», sino «depende de esto, y aquí ocurre así». Ancla el contraste en *Customer Centricity* y *Hillstrom's Multichannel Forensics*. |
