...
**tarea automatizable** se entiende aquí como **actividad concreta con entrada y salida definidas que un sistema puede ejecutar**. Es la pieza desde la que se inicia el análisis de mapa de IA comercial: antes de «inventariar tareas comerciales candidatas», hay que poder señalar qué cambia en la operación si el concepto está presente y qué debería observarse si no lo está.

La lectura rectora de este bloque es Stuart Russell y Peter Norvig — *Artificial Intelligence: A Modern Approach* (2021, 4.ª ed.). **Lente que aporta:** marco formal de agentes, entornos y medidas de desempeño. Úsala sin convertirla en dogma: escribe una proposición de la obra que apoye tu diagnóstico, una condición del caso que la limite y una consecuencia práctica. La evidencia mínima es **casos de uso con criterio de éxito**; regístrala con periodo, unidad, población y línea base.
...
**Definición operacional:** clasificación según lo que hace: genera, clasifica, predice, recupera o actúa. Su valor está en distinguirlo de **tarea automatizable**. En una decisión real, clasificar mal una situación cambia la intervención: se asigna presupuesto donde faltaba diagnóstico, se mide un resultado cuando había que observar un proceso, o se trata una restricción como si fuera una preferencia.

Contrasta el problema con NIST — *AI Risk Management Framework 1.0* (2023) —**lente:** gobernanza de riesgo de IA: mapear, medir, gestionar y gobernar—. Formula dos mini-casos: uno que satisface la definición de **tipo de sistema** y otro que sólo se le parece en la superficie. Después pregunta qué señal los distingue; **costo del error por caso** es candidata, pero debe combinarse con evidencia cualitativa cuando el fenómeno no es directamente medible.
...
**criterio de éxito** significa **definición operacional de qué significa que el sistema funcione bien**. El problema ya no es definirlo sino medirlo: qué contar, en qué ventana, con qué denominador, contra qué línea base y con qué segmentación. Una métrica útil conserva contexto suficiente para no confundir una mejora local con una mejora del sistema.
...
**Definición:** persona que responde por el resultado con independencia de la automatización. Este concepto obliga a abandonar la idea de que mapa de IA comercial tiene una solución gratuita. Toda intervención consume caja, tiempo, atención del equipo, capacidad de la operación, reputación o tolerancia al riesgo. Por eso, antes de «asignar responsable humano por cada uso», se comparan al menos dos alternativas plausibles y se explicita qué se sacrifica en cada una.

Andrew Ng — *Machine Learning Yearning* (2018) —**lente:** diagnóstico de sistemas de aprendizaje y priorización de mejoras— sirve para construir una matriz `beneficio esperado / costo / reversibilidad / stakeholder afectado / señal temprana`. La evidencia **usos con responsable asignado** ayuda a detectar si el trade-off está ocurriendo como se esperaba, pero no elimina la obligación de observar efectos laterales fuera del indicador principal.
...
Andrew Ng — *Machine Learning Yearning* (2018) sirve para contrastar la recomendación final desde otro lente: diagnóstico de sistemas de aprendizaje y priorización de mejoras. La frontera de esta clase es explícita: La IA no resuelve problemas de proceso ni de datos: los amplifica. Automatizar sobre información de mala calidad produce errores más rápido. Conviértela en una regla operativa con el formato `si ocurre X → no aplicar automáticamente → consultar, escalar o revalidar`.
...
2. Ejecuta la secuencia **inventariar tareas comerciales candidatas → clasificar cada una por tipo de sistema requerido → definir el criterio de éxito y el costo del error → asignar responsable humano por cada uso → priorizar por valor y por riesgo controlable** y adjunta evidencia en cada transición.
3. Construye la ficha de medición de **casos de uso con criterio de éxito**; si el dato no existe, diseña cómo obtenerlo y cuánto costaría.
