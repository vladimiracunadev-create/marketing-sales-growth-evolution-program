...
sources: ["reichheld", "cialdini", "mehta", "godin"]
updated: 2026-08-18
---
...
1. **Distinguir** `momento de referencia`, `petición específica`, `facilitación` y `reciprocidad no monetaria` por sus observables y no por su definición memorizada.
2. **Explicar** por qué esas distinciones cambian una decisión concreta dentro de **Prospección y generación de demanda**.
3. **Aplicar** la secuencia **identificar clientes con resultado acreditado → elegir el momento de la petición → solicitar una introducción específica → facilitar el mensaje que el cliente enviará → medir conversión y agradecer el resultado** conservando supuestos, alternativas descartadas y trazabilidad.
4. **Operacionalizar** **tasa de clientes que refieren**, **conversión de referidos** y **costo por cliente referido** indicando numerador, denominador, ventana, fuente y uso permitido.
...
**momento de referencia** se entiende aquí como **instante en que el cliente ha obtenido resultado y está dispuesto a referir**. Es la pieza desde la que se inicia el análisis de referidos: antes de «identificar clientes con resultado acreditado», hay que poder señalar qué cambia en la operación si el concepto está presente y qué debería observarse si no lo está.

La lectura rectora de este bloque es Fred Reichheld, Darci Darnell y Maureen Burns — *Winning on Purpose* (2021). **Lente que aporta:** lealtad, economía del cliente ganado y usos correctos e incorrectos del NPS. Úsala sin convertirla en dogma: escribe una proposición de la obra que apoye tu diagnóstico, una condición del caso que la limite y una consecuencia práctica. La evidencia mínima es **tasa de clientes que refieren**; regístrala con periodo, unidad, población y línea base.
...
**Definición operacional:** solicitud de introducción a una persona u organización identificada. Su valor está en distinguirlo de **momento de referencia**. En una decisión real, clasificar mal una situación cambia la intervención: se asigna presupuesto donde faltaba diagnóstico, se mide un resultado cuando había que observar un proceso, o se trata una restricción como si fuera una preferencia.

Contrasta el problema con Robert B. Cialdini — *Influence: The Psychology of Persuasion, New and Expanded* (2021) —**lente:** principios de influencia y su uso ético en contextos comerciales—. Formula dos mini-casos: uno que satisface la definición de **petición específica** y otro que sólo se le parece en la superficie. Después pregunta qué señal los distingue; **conversión de referidos** es candidata, pero debe combinarse con evidencia cualitativa cuando el fenómeno no es directamente medible.
...
**facilitación** significa **material que permite al cliente referir sin esfuerzo ni riesgo**. El problema ya no es definirlo sino medirlo: qué contar, en qué ventana, con qué denominador, contra qué línea base y con qué segmentación. Una métrica útil conserva contexto suficiente para no confundir una mejora local con una mejora del sistema.
...
**Definición:** reconocimiento o beneficio que no compromete la credibilidad de la referencia. Este concepto obliga a abandonar la idea de que referidos tiene una solución gratuita. Toda intervención consume caja, tiempo, atención del equipo, capacidad de la operación, reputación o tolerancia al riesgo. Por eso, antes de «facilitar el mensaje que el cliente enviará», se comparan al menos dos alternativas plausibles y se explicita qué se sacrifica en cada una.

Seth Godin — *This Is Marketing* (2018) —**lente:** marketing como servicio a un público mínimo viable y construcción de confianza— sirve para construir una matriz `beneficio esperado / costo / reversibilidad / stakeholder afectado / señal temprana`. La evidencia **costo por cliente referido** ayuda a detectar si el trade-off está ocurriendo como se esperaba, pero no elimina la obligación de observar efectos laterales fuera del indicador principal.
...
Seth Godin — *This Is Marketing* (2018) sirve para contrastar la recomendación final desde otro lente: marketing como servicio a un público mínimo viable y construcción de confianza. La frontera de esta clase es explícita: Incentivar económicamente las referencias puede comprometer su credibilidad y, si no se declara, constituye publicidad encubierta. Conviértela en una regla operativa con el formato `si ocurre X → no aplicar automáticamente → consultar, escalar o revalidar`.
...
2. Ejecuta la secuencia **identificar clientes con resultado acreditado → elegir el momento de la petición → solicitar una introducción específica → facilitar el mensaje que el cliente enviará → medir conversión y agradecer el resultado** y adjunta evidencia en cada transición.
3. Construye la ficha de medición de **tasa de clientes que refieren**; si el dato no existe, diseña cómo obtenerlo y cuánto costaría.
...
1. Explica la diferencia entre **momento de referencia** y **petición específica** con un ejemplo donde elegir mal cambie la decisión.
2. ¿Qué observarías para validar **facilitación** y qué observación te obligaría a rechazar tu interpretación?
3. Aplica «identificar clientes con resultado acreditado» al caso de la clase. ¿Qué dato sigue faltando?
4. ¿Por qué **tasa de clientes que refieren** no basta por sí sola para atribuir causalidad?
5. Compara dos fuentes de la lectura comparada: ¿dónde llevarían a recomendaciones distintas?
6. ¿Qué decisión equivocada se produciría si se ignora este límite: «Incentivar económicamente las referencias puede comprometer su credibilidad y, si no se declara, constituye publicidad encubierta»?
...
- `ficha-metricas.md` — definición operacional de **tasa de clientes que refieren**, **conversión de referidos** y **costo por cliente referido** con fuente, ventana y lectura prohibida.
...
← [Clase 07 · Networking](class-07-networking.md) · [Índice de la parte](README.md) · [Clase 09 · Eventos y comunidades](class-09-eventos-y-comunidades.md) →
